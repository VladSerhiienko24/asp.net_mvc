...
namespace Tours_1._0.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model2)
        {
            if (!ModelState.IsValid)
            {
                //return View("UserSettings", "Account", model);
                return RedirectToAction("UserSettings", "Account");
            }
            var result = await UserManager.ChangePasswordAsync(User.Identity.GetUserId(), model2.OldPassword, model2.NewPassword);
            if (result.Succeeded)
            {
                ...
                return RedirectToAction("UserSettings", "Account", new { Message = ManageMessageId.ChangePasswordSuccess });
            }

            AddErrors(result);
            return RedirectToAction("UserSettings", "Account", new { Message = ManageMessageId.FalsePassword });
            //return View("UserSettings", "Account", model);
        }
